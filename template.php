...
//}
/*
function zurb_subfloor_form_alter(&$form, &$form_state, $form_id) {
...
function zurb_subfloor_preprocess_html(&$vars) {
  $node = menu_get_object();
  dpm($node);
  if ($node && $node->nid && $node->type=='codefiddle') {
    if(isset($_GET['embed'])){
      $vars['theme_hook_suggestions'][] = 'html__codefiddle_embed';
    }
    else{
      $vars['theme_hook_suggestions'][] = 'html__codefiddle';
    }
  }
}

function zurb_subfloor_preprocess_page(&$vars) {
  if (!empty($vars['node']) && !empty($vars['node']->type) && $vars['node']->type=='codefiddle') {
    //$node->type;
    if(isset($_GET['embed'])){
      $vars['theme_hook_suggestions'][] = 'page__codefiddle_embed';
    }
    else{
      $vars['theme_hook_suggestions'][] = 'page__codefiddle';
    }
  }
}
